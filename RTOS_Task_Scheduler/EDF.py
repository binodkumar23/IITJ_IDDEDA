...
def intialise_heap(hea,deadline,processName,executionTime,table):
    for i in range(len(processName)):
        hea.push_heap( ( deadline[i],-executionTime[i],-table[processName[i]] ) )
...
if  U<=1:
    print("Uilization is less than 1 thus we can use this scheduling algo here.")
else:
...
intialise_heap(heap1,di,task,ci,table0)

# main logic
while time < simultaion_time:
    
    # checking if new process should be  added to heap or not 
    while (not heap2.isEmpty() and (heap2.get_top()[0] - ti[-heap2.get_top()[-1] - 1]) <= time):
        heap1.push_heap(heap2.pop_heap())
    ...
    x,y,z = heap1.pop_heap()
    summary.append((table1[-z],time,time+1))
    y = y+1
    if y == 0:
        heap2.push_heap(( x+ti[-z-1],-ci[-z-1],z ))
    else:
        heap1.push_heap((x,y,z))
